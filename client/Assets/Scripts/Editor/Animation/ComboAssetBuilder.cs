...
/// 一键生成一套**可玩的近战连招 demo**，落盘到 Resources/Character/Player/Skills/，并配合 knife 的 ComboGraphPath 跑起来：
...
public static class ComboAssetBuilder
{
    ...
    {
        CharacterResPath.ToAsset(CharacterResPath.PlayerMelee),
        CharacterResPath.ToAsset(CharacterResPath.PlayerKnife),
        CharacterResPath.ToAsset(CharacterResPath.PlayerKnifeV),
    };
    ...
    public static void Build()
    {
        ...
        string SkillAsset(string name) => CharacterResPath.ToAsset(CharacterResPath.PlayerRoot + "Skills/" + name);
        ...
        CreateOrReplace(atk1,  SkillAsset("Knife_Atk1"));
        CreateOrReplace(atk2,  SkillAsset("Knife_Atk2"));
        CreateOrReplace(atk3,  SkillAsset("Knife_Atk3"));
        CreateOrReplace(heavy, SkillAsset("Knife_Heavy"));
        ...
        CreateOrReplace(graph, CharacterResPath.ToAsset(CharacterResPath.KnifeCombo));

        AssetDatabase.SaveAssets();
        AssetDatabase.Refresh();
        Debug.Log($"[ComboAssetBuilder] 完成（复用 {clips.Count} 个 clip）：Knife_Atk1/2/3 + Knife_Heavy + KnifeCombo 已生成到 Resources/Character/Player/Skills/。\n" +
                  "切到 knife（数字键 5）：左键连点=三连段，左左V=重击收尾，单V=重击。knife 的 ComboGraphPath 已指向 Character/Player/Skills/KnifeCombo。");
    }
