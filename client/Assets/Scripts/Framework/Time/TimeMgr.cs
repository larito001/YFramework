...
public class Timers
{
    ...
    public void Add(float interval, int repeat, TimerCallback callback, bool isCallImmediately = false)
    {
        Add(interval, repeat, callback, null, isCallImmediately);
    }

    public void Add(float interval, TimerCallback callback)
    {
        Add(interval, 1, callback, null);
    }
...
#if UNITY_PROFILER_TEST
            UnityEngine.Profiling.Profiler.BeginSample("Timers.Update foreach");
#endif
        if (_items.Count > 0)
        {
            ...
            while (iter.MoveNext())
            {
                ...
                if (i.callback != null)
                {
                    if (catchCallbackExceptions)
                    {
                        ...
                        {
#if UNITY_PROFILER_TEST
								UnityEngine.Profiling.Profiler.BeginSample($"Timers callback = {i.callback.Method.DeclaringType.FullName}_{i.callback.Method.DeclaringType.Name}");
#endif
                            i.callback(i.param);
#if UNITY_PROFILER_TEST
                        ...
                        {
                            i.deleted = true;
                            Debug.LogError("FGui:TimerError(internal=" + i.interval + ", repeat=" + i.repeat +
                                           ",functionName" + i.callback.Method.Name + ") callback error > " +
                                           e.StackTrace);
                        }
                    }
                    else
...
#if UNITY_PROFILER_TEST
            UnityEngine.Profiling.Profiler.BeginSample("Timers.Update remove");
#endif
...
#if UNITY_PROFILER_TEST
            UnityEngine.Profiling.Profiler.BeginSample("Timers.Update add");
#endif
        if (_toAdd.Count > 0)
        {
