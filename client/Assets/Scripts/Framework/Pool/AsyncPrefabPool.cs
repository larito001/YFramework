...
/// <summary>
/// 异步 prefab 池：按字符串路径池化 <see cref="ResMgr"/> 异步加载的 prefab。
/// 适用：UI 飘字、低频特效、拾取物、可破坏物等"加载异步 + 出现频率不高 + 数量浮动"的对象。
/// 不适用：TPS 子弹、casing、弹孔等 hot path 对象——首帧延迟会糊手感，请用 PrefabPool&lt;T&gt; 或 UnityEngine.Pool.ObjectPool&lt;T&gt;。
/// </summary>
public class AsyncPrefabPool : IGameService
{
    ...
    public void Init(GameContext ctx)
    {
        ...
        poolRoot = new GameObject("AsyncPrefabPoolRoot").transform;
        GameObject.DontDestroyOnLoad(poolRoot.gameObject);

        AsyncPooledObject.Configure(this);
    ...
    {
        Clear();
        AsyncPooledObject.Configure(null);
        resMgr = null;
    }
