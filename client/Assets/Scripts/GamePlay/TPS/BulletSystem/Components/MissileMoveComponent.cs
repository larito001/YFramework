...
/// 继承 <see cref="SegmentRaycastMoveBase"/>：raycast / 命中扣血 / Despawn 由基类提供，
/// 本类只负责"贝塞尔段是怎么走的" + "段末空爆"。
///
/// 朝向：每帧把"本段位移/dt"写回 Owner.Velocity，BulletView 用这个值算 LookRotation，
///       看起来就是机头沿切线方向飞。
///
/// 配方由 <see cref="BezierMissileEffect"/> 在 spawn 时填好；本组件不算控制点。
/// </summary>
public class MissileMoveComponent : SegmentRaycastMoveBase
{
    ...
    public override void Tick(float dt)
    {
        ...
        if (t >= 1f)
        {
            // 段尾再做一次 raycast，覆盖落点附近近距离命中。命中则 HandleHitAndDespawn 已处理，直接返回。
            if (CastSegment(Owner.Position, End, out var hit))
            {
#if UNITY_EDITOR
        ...
        var next = Bezier(t);
        if (CastSegment(prev, next, out var midHit))
        {
#if UNITY_EDITOR
            Debug.Log($"[Missile] hit {midHit.collider.name} @ {midHit.distance:F2}m, dmg={Owner.Damage}");
#endif
            HandleHitAndDespawn(midHit);
            return;
        }
