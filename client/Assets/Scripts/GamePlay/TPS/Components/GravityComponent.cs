...
///   - 最后把 vy 写到 Owner.WishVelocity.y，View.LateUpdate 用 Controller.Move 应用。
///
/// Add 顺序：放在所有写 x/z 的组件（Move / Melee）**之后**，保证 Tick 末 WishVelocity.y 由本组件最终决定。
/// 没有 Move 的"站桩 actor"（Dummy）只需 Add 本组件就能正确落地。
///
/// 后续要做"跳跃"：新建 JumpComponent 在事件回调里设字段触发瞬时 impulse，
/// 本组件优先消费该字段覆写 vy 即可（暂未实现）。
/// </summary>
public class GravityComponent : IActorComponent
{
