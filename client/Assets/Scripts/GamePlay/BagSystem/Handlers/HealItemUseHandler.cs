...
/// <summary>
/// 治疗类消耗品的使用逻辑示例：使用时给当前玩家回血。
/// 这是 <see cref="IItemUseHandler"/> 的参考实现——演示「效果与配表解耦、按物品 id 注册」的用法。
///
/// 玩家在 handler 内**延迟查找**（每次使用时通过 CharacterManager.Player 取），
/// 所以注册时机不要求玩家已 spawn；玩家死亡/重生切换实例也能正确命中最新玩家。
///
/// 回血量当前由构造参数传入（不同药水注册不同数值）。若想完全数据驱动，
/// 在 item.xlsx 加一列（如 useParam:int）重跑打表工具后改读 <c>context.Item.UseParam</c> 即可。
/// </summary>
