...
public class ThirdPlayerMoveCtrl : MonoBehaviour
{
    ...
    float jumpHeight = 2f;//踢啊欧俄高度
    [SerializeField, Range(0, 5)]
    int maxAirJumps = 0;//跳跃段数
    [SerializeField, Range(0f, 90f)]
    float maxGroundAngle = 25f, //允许最大爬坡角度
        maxStairsAngle = 50f;//允许最大楼梯角度
    
    [SerializeField, Range(0f, 100f)]
    float maxSnapSpeed = 100f;//起飞速度，不大于这个速度会被牢牢吸附在平面
    
    [SerializeField, Min(0f)]
    float probeDistance = 1f;//当球体下方有地面时，无论距离有多远，我们都会进行捕捉
    [SerializeField]
    LayerMask probeMask = -1,//检测的层
        stairsMask = -1;//楼梯的曾
    ...
    void EvaluateCollision(Collision collision)
    {
        ...
        for (int i = 0; i < collision.contactCount; i++) {
            ...
            }else if (normal.y > -0.01f) {//如果接触到了墙则，记录法线
                steepContactCount += 1;
                steepNormal += normal;
            }
        }
